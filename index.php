...
<html>
<head>
	<title>Trying for open_ssl_encryption</title>
	...
	$(document).ready(function() {
	    ...
	    $("#register").click(function() {
	        $.ajax({
	        	type: "POST",
	        	url: "http://localhost/encryption-php7/register.php",
	    ...
	    function getAllUsers() {
	        $.ajax({
	        	type: "GET",
	        	url: "http://localhost/encryption-php7/display.php",
	        	success: function(result) {
	        		if(result.resultCode == "Success") { 
	        			$("#loading").html("Loaded the table");
	        				$("#user-table").css("visibility","visible");
		        			for(var i = 0; i < result.records.length; i++) {
		        				$('#user-table tbody').append("<tr><td>" +result.records[i].id+ "</td><td>" +result.records[i].email+ "</td><td>" +result.records[i].password+ "</td></tr> ");
							}
	        		} else {
	        			$("#loading").html(result.message);
